...
if [[ $ACTION == "pause" ]]; then
    FUNC_SIG="0x60686fbf"
    ENCODED_PARAMS=$(cast calldata "pauseViaSafe(string[],bool)" "$MODULE_NAMES_FORMATTED" "$SAFE_SEND" || exit 1)

elif [[ $ACTION == "unpause" ]]; then
    FUNC_SIG="0xb1034f38"
    ENCODED_PARAMS=$(cast calldata "unpauseViaSafe(string[],bool)" "$MODULE_NAMES_FORMATTED" "$SAFE_SEND" || exit 1)
else
    echo "Invalid action"
    exit 1
fi

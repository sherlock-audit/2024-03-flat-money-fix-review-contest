...
# Function signature of the function "deployModules(string[],bool)"
FUNC_SIG="0x6b7954fb"

# Get abi-encoded parameters for the function.
# Don't proceed if there is an error after calling cast.
ENCODED_PARAMS=$(cast calldata "deployModules(string[],bool)" "$MODULE_NAMES_FORMATTED" "$SAFE_SEND" || exit 1)
